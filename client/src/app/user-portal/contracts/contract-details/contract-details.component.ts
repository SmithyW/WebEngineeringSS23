...
export class ContractDetailsComponent implements OnInit, OnDestroy {
	...
	constructor(private route: ActivatedRoute, private rest: RestService, private dateUtil: DateTimeUtilsService) {}
	...
	private initForm() {
		this.beginControl = new FormControl(
			this.dateUtil.formatDate(this.contract?.begin || "", "YYYY-MM-DD"),
			Validators.required
		);
		this.endControl = new FormControl(
			this.dateUtil.formatDate(this.contract?.end || "", "YYYY-MM-DD"),
			Validators.required
		);
  ...
  private updateForm() {
		this.beginControl.setValue(this.dateUtil.formatDate(this.contract?.begin || "", "YYYY-MM-DD"));
		this.endControl.setValue(
			this.dateUtil.formatDate(this.contract?.end || "", "YYYY-MM-DD")
		);
		this.supervisorControl.setValue(this.contract?.supervisor);
    this.timePerWeekdayArray.clear();
		if (this.contract?.timePerWeekday) {
			for (let timePerWeekday of this.contract?.timePerWeekday) {
				this.timePerWeekdayArray.push(
					new FormGroup<{ weekday: FormControl<EWeekday | null>; time: FormControl<number | null> }>({
	...
	createEmptyTimePerWeekdayItem(): FormGroup<{
		weekday: FormControl<EWeekday | null>;
	...
	private setContract(contract: ContractData) {
		this.contract = contract;
		this.contract.begin = this.dateUtil.ensureDate(contract.begin);
		this.contract.end = this.dateUtil.ensureDate(contract.end);
	}
}
